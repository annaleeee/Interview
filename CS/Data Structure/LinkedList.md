...
- append + insert 메서드
    ...
    class LinkedList{
      ...
      append(newData){
        ...
        if(this.head === null){
         ...
         let currentNode = this.head; // 첫번째 노드
         while(currentNode.next != null){
              currentNode = currentNode.next // current 이동 (list를 순회)
         }
         currentNode.next = newNode; // 자기 자신을 참조 / 두번째 노드일 때 while문 걸림
      }
    
    	insertAt(newData, index){
        let newNode = new Node(newData);
        if(index == 0){ 
          // newNode.next = newNode; // 새로운 노드의 next 값은 현재 연결리스트의 head
    ...
    linkedList.append("hello");
    linkedList.append("bye");
    linkedList.insertAt("happy");
    linkedList.insertAt("sad");
    console.log(linkedList.head); // 첫번째 노드에 접근
    ```
